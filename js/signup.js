(function() {
  ...
  if (password && strengthBar && strengthText) {
    password.addEventListener('input', function() {
      ...
      if (strength <= 1) {
        ...
        strengthText.className = 'text-xs text-pink-600 mt-1';
      } else if (strength <= 2) {
        strengthBar.classList.add('strength-fair');
        strengthText.textContent = 'Fair';
        strengthText.className = 'text-xs text-pink-500 mt-1';
      } else if (strength <= 3) {
        strengthBar.classList.add('strength-good');
        strengthText.textContent = 'Good';
        strengthText.className = 'text-xs text-purple-500 mt-1';
      } else {
        strengthBar.classList.add('strength-strong');
        strengthText.textContent = 'Strong';
        strengthText.className = 'text-xs text-purple-600 mt-1';
      }
    });
  }
  ...
  function showError(el, errEl) {
    if (el) { el.classList.add('border-pink-500', 'dark:border-pink-400'); }
    if (errEl) { errEl.classList.remove('hidden'); }
  }
  function clearError(el, errEl) {
    if (el) { el.classList.remove('border-pink-500', 'dark:border-pink-400'); }
