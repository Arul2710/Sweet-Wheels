class Navbar {
  ...
  getNavLinks() {
    const commonLinks = [
      ...
      { label: 'Cotton Candy', href: 'cotton-candy.html' },
      { label: 'Funnel Cakes', href: 'funnel-cakes.html' },
  ...
  render() {
    ...
    navbar.innerHTML = `
      <nav class="fixed top-0 left-0 w-full z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg border-b border-pink-100 dark:border-gray-700 transition-all duration-300" id="mainNav" aria-label="Main navigation">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex items-center justify-between h-16 md:h-20">
            <a href="index.html" class="flex items-center space-x-2 rtl:space-x-reverse group shrink-0" aria-label="Sweet Wheels - Cotton Candy &amp; Funnel Cake Food Truck - Home">
              <div class="relative w-10 h-10 md:w-11 md:h-11 bg-gradient-to-br from-pink-500 to-purple-500 rounded-full flex items-center justify-center shadow-md group-hover:shadow-lg transition-all duration-300 group-hover:scale-105">
                <span class="text-white text-lg md:text-xl leading-none" aria-hidden="true">&#127853;</span>
                <span class="absolute -bottom-1 -right-1 w-5 h-5 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center text-[10px] md:text-[11px] shadow-sm" aria-hidden="true">&#128666;</span>
              </div>
            ...
            <div class="flex items-center gap-1">
              <div class="hidden lg:flex items-center gap-1">
                <button id="darkModeToggle" class="p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-pink-50 dark:hover:bg-gray-800 transition-all duration-200 focus-visible:outline-2 focus-visible:outline-pink-500" aria-label="Toggle dark mode" title="Toggle Dark Mode">
                ...
                <button id="ltrToggle" class="relative w-[68px] h-7 rounded-full transition-all duration-300 focus-visible:outline-2 focus-visible:outline-pink-500" aria-label="Switch to RTL" title="Switch to RTL">
                    <span class="absolute inset-0 rounded-full bg-gray-200 dark:bg-gray-700"></span>
                    <span id="ltrToggleTrack" class="absolute inset-0 rounded-full bg-gradient-to-r from-pink-500 to-purple-500 opacity-0 transition-opacity duration-300"></span>
              ...
              <button id="mobileMenuBtn" class="lg:hidden p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-pink-50 dark:hover:bg-gray-800 transition-all duration-200 focus-visible:outline-2 focus-visible:outline-pink-500" aria-label="Open menu" aria-expanded="false" aria-controls="mobileMenu">
        ...
        <div id="mobileMenu" class="lg:hidden max-h-0 opacity-0 overflow-hidden transition-all duration-400 ease-in-out bg-white dark:bg-gray-900 border-t border-pink-100 dark:border-gray-700" role="menu" aria-label="Mobile navigation">
          <div class="px-4 py-3 space-y-1">
            ${commonLinks
              .map((link) => {
                return `
                  <a href="${link.href}" class="block px-3 py-2.5 text-sm font-medium ${isActive(link.href)} rounded-lg transition-all duration-200 hover:bg-pink-50 dark:hover:bg-gray-800 ${link.class || ''}" ${link.id ? `id="${link.id}-mob"` : ''} role="menuitem">
                    ${link.label}
                  </a>
                `;
              })
              .join('')}
            <hr class="border-pink-100 dark:border-gray-700 my-3" role="separator" aria-hidden="true">
            ${authLinks
              .map(
                (link) => `
              <a href="${link.href}" class="block px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 ${link.href === 'signup.html' ? 'btn-primary text-white shadow-md hover:shadow-lg text-center' : `${isActive(link.href)} hover:bg-pink-50 dark:hover:bg-gray-800 ${link.class || ''}`}" ${link.id ? `id="${link.id}-mob"` : ''} role="menuitem">
                ${link.label}
              </a>
            `
              )
            ...
            <div class="flex items-center justify-center gap-3 pt-2 pb-1">
              <button id="darkModeToggle-mob" class="flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-pink-50 dark:hover:bg-gray-800 transition-all duration-200" aria-label="Toggle dark mode">
                <svg class="w-5 h-5 block dark:hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/>
                </svg>
                <svg class="w-5 h-5 hidden dark:block" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              <button id="ltrToggle-mob" class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-pink-50 dark:hover:bg-gray-800 transition-all duration-200" aria-label="Toggle direction">
                <span>Direction</span>
                <span class="relative w-[68px] h-7 rounded-full pointer-events-none">
                  <span class="absolute inset-0 rounded-full bg-gray-200 dark:bg-gray-700"></span>
                  <span id="ltrToggleTrack-mob" class="absolute inset-0 rounded-full bg-gradient-to-r from-pink-500 to-purple-500 opacity-0 transition-opacity duration-300"></span>
                  <span class="relative flex items-center justify-between px-2.5 h-full text-[10px] font-bold uppercase leading-none tracking-wide">
